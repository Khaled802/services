...
from .models import Bank, BaseService, Hotal, Restraunt
...
class HotalSerializer(BaseServiceSerializer):     
...
class SevicesSerializer:
    ...
    serializers = {
        Bank: BankSerializer,
        Hotal: HotalSerializer,
        Restraunt: RestrauntSerializer

    } 

    @classmethod
    def serialize_service(cls, service: BaseService, objects: list) -> BaseServiceSerializer:
        service_serializer = cls.serializers.get(service, None)
        if service_serializer is None:
            # to force you to add the model and its serializer in the serializer dict
            raise NotImplementedError(f'should add {service} to serializers dict in {cls.__name__} class')
